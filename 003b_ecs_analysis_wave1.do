...
foreach x in frrisk10 frsim10 frcat frsimcat WHO_nolab WHO_gen WHObmi_cat WHOgen_cat ascvd10 ascvd2_cat {
...
** LOW CATEGORY
foreach x in frcat frsimcat WHObmi_cat WHOgen_cat ascvd2_cat {
        gen `x'_low=`x' 
...
preserve
    collapse (mean) ascvd2_cat_high, by(indicator2)
    replace indicator2=3 
    rename ascvd2_cat_high high 
    save `s20', replace
restore
...
preserve
    collapse (mean) ascvd2_cat_int, by(indicator2)
    replace indicator2=3 
    rename ascvd2_cat_int inter 
    save `s25', replace 
restore
...
preserve
    collapse (mean) ascvd2_cat_low, by(indicator2)
    replace indicator2=3 
    rename ascvd2_cat_low low 
    save `s30', replace
restore
